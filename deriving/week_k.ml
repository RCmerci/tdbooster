...
let week_k (data_list : Loader.Type.raw_data) : Loader.Type.raw_data =
  List.fold data_list ~init:(0, None, [])
    ~f:(fun (last_week_num, (week_data : Loader.Type.raw_data_elem option), r)
            current
            ->
      let week_num = Date.week_number (Loader.Type.date current) in
      if last_week_num <> week_num then
        (* new week *)
        let percent_change =
          match r with
          | Some (last_week_data : Loader.Type.raw_data_elem) :: _ ->
        ...
        in
        ( week_num
        , ( week_data >>= fun (week_data' : Loader.Type.raw_data_elem) ->
            Some
              { week_data' with
                close = current.close
              ; high =
