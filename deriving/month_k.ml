         ...
         ->
           ...
           | Some last_m when Month.(month <> last_m) ->
             let percent_change = match r with
               | (Some (last_month_data:Loader.Type.raw_data_elem))::_ -> (current.close -. last_month_data.close) /. last_month_data.close
               | _ -> 0.
             in
             ( (* new month *)
               Some month
           ...
           | Some _ ->
             ...
             , ( month_data
                 >>= fun month_data' ->
                 let percent_change = match r with
                   | (Some last_month_data)::_ -> (current.close -. last_month_data.close) /. last_month_data.close
                   | _ -> 0.
                 in                 
                 Some { month_data' with
                   close=current.close;
                   high=(if current.high > month_data'.high then
                           current.high
                         else month_data'.high);
                   low= (if current.low < month_data'.low then
                           current.low
                         else month_data'.low);
                   ttm=current.ttm;
