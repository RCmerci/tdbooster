...
let smma n (closing_data_list : float array) : float list =
...
let rsi_all_days n (closing_data_list:float array): float list =
  let l = Array.map2_exn
      (Array.sub closing_data_list ~pos:0 ~len:(Array.length closing_data_list - 1))
      (Array.sub closing_data_list ~pos:1 ~len:(Array.length closing_data_list - 1))
      ~f:(fun a b -> if a > b then (0., a  -. b) else (b -. a, 0.)) in
  let up, down = Array.unzip l in
  let up' = smma n up in
