...
let kdj_all_days n (data: Loader.Type.raw_data list) =
  ...
  let rsv_list = match List.map2 data hl_list ~f:(fun e (high, low) -> rsv e.closing low high) with
  ...
  | List.Or_unequal_lengths.Unequal_lengths -> failwith "kdj_all_days"
  in
  match List.zip (List.map data ~f:(fun e -> e.date)) kdj_list with
  | List.Or_unequal_lengths.Ok e -> e
  | List.Or_unequal_lengths.Unequal_lengths -> failwith "kdj_all_days"
...
let%test "test-kdj" =
  ...
  in
  let datal' = List.sub datal ~pos:4000 ~len:(List.length datal - 4001) in
  List.last_exn (kdj_all_days 9 datal') |> fun (_, (k,d,j))-> int_of_float k = 56 && int_of_float d = 65 && int_of_float j = 38

let kdj n (data_list:Loader.Type.raw_data list): (Date.t * (float * float * float)) list = kdj_all_days n data_list
