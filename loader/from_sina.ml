...
module Futures = struct
  let get_futrues_data ~output_dir ~code =
    Cohttp_lwt_unix.Client.get
      (Uri.of_string
         (Printf.sprintf
            "https://stock2.finance.sina.com.cn/futures/api/jsonp.php/var%%20_XX=/GlobalFuturesService.getGlobalFuturesDailyKLine?symbol=%s"
            code))
    >>= fun (_, body) ->
  ...
  let read_from_file ~output_dir ~code : Type.raw_data =
    ...
    List.map l ~f:(fun e ->
        ...
        | `Assoc al ->
          ...
          ( { date =
                List.Assoc.find_exn al ~equal:String.( = ) "date"
                |> Yojson.Safe.Util.to_string |> Date.of_string
            ; opening =
                List.Assoc.find_exn al ~equal:String.( = ) "open"
                |> Yojson.Safe.Util.to_string |> Float.of_string
            ; high =
                List.Assoc.find_exn al ~equal:String.( = ) "high"
                |> Yojson.Safe.Util.to_string |> Float.of_string
            ; low =
                List.Assoc.find_exn al ~equal:String.( = ) "low"
                |> Yojson.Safe.Util.to_string |> Float.of_string
            ; close =
                List.Assoc.find_exn al ~equal:String.( = ) "close"
                |> Yojson.Safe.Util.to_string |> Float.of_string
            ; ttm = 0.
            ; days = 1
            ; percent_change = 0.
