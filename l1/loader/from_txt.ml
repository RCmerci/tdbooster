    ...
    let h =
      match
        Hashtbl.create_mapped
          (module Date)
          ~get_key:(fun (x : Type.raw_data_elem) -> x.date)
          ~get_data:(fun x -> x)
    ...
    let h' =
      match
        Hashtbl.create_mapped
          (module Date)
          ~get_key:(fun x -> fst x)
          ~get_data:(fun x -> snd x)
          (List.map t' ~f:parse_ttm_line |> List.filter_opt)
      with
      | `Duplicate_keys _ -> failwith "Hashtbl.create_mapped ttm"
      | `Ok h -> h
    in
    Hashtbl.merge h h' ~f:(fun ~key:_ ->
      function
      | `Left x -> Some x
      | `Both (x, y) -> Some { x with ttm = y }
      | _ -> None)
    |> Hashtbl.to_alist |> List.unzip |> snd
    |> List.sort
         ~compare:(fun (a : Type.raw_data_elem) (b : Type.raw_data_elem) ->
           Date.compare a.date b.date)
  | _ -> failwith "empty data"

let read_from_file file ttm_file : Type.raw_data =
  let lines = In_channel.read_lines file in
  let ttm_lines = In_channel.read_lines ttm_file in
  match (lines, ttm_lines) with
  | _headline :: _t, _headline' :: _t' -> read_from_string_lines lines ttm_lines
  | _ ->
