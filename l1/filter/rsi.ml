...
let golden_cross_points diff_list =
  let points =
    List.foldi ~init:[] diff_list ~f:(fun i r (diff1, diff2) ->
  ...
  List.filter points ~f:(fun i ->
      ...
      | Some (before1_diff1, before1_diff2), Some (before2_diff1, before2_diff2)
        ->
        curr_diff1 <= before1_diff1
        && curr_diff2 <= before1_diff2
        && curr_diff1 <= before2_diff1
        && curr_diff2 <= before2_diff2)
