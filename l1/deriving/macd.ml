...
let dif ~ema12_list ~ema26_list : float list option =
  ...
  with
  (* 第一日的 dif 为 0 *)
  | List.Or_unequal_lengths.Ok (_h :: t) -> Some (0. :: t)
  | List.Or_unequal_lengths.Ok _ -> Some [ 0. ]
...
let macd ~dif_list ~dea_list =
  ...
  | List.Or_unequal_lengths.Ok v -> Some v
  | List.Or_unequal_lengths.Unequal_lengths ->
...
let macd_dif_dea (data_list : L1_loader.Type.raw_data) :
  ...
  dif ~ema12_list ~ema26_list >>= fun dif_list ->
  let dea_list = dea ~dif_list in
  macd ~dif_list ~dea_list >>= fun macd_list ->
