...
let week_k_buy c _ month_ctx :
  ...
  if Price_status.status_combine_1 c then
    log (Printf.sprintf "[%s] status_combine_1=true" (Data_cursor.datestring c))
    >>= fun _ -> return (Buy (c, None, month_ctx))
  else
    return (Buy_continue None)

let day_k_buy c _ctx _week_ctx :
    (ctx, day_to_sell_ctx) buy_action Log_warning.LogAndWarnWriter.m =
  log (Printf.sprintf "[%s] buy" (Data_cursor.datestring c)) >>= fun _ ->
  return
    (Buy (c, Some (Loader.Type.close (Data_cursor.current c).raw_data), ()))

let rec sell_aux next_c week_c :
    (Data_cursor.t * float) option Log_warning.LogAndWarnWriter.m =
  let week_c' =
    Option.value_exn (Data_cursor.goto_date week_c (Data_cursor.date next_c))
  in
  if Price_status.status_combine_2 week_c' then
    logf "[%s] status_combine_2" (Data_cursor.datestring week_c') >>= fun _ ->
    return
      (Some (week_c', Loader.Type.close (Data_cursor.current week_c').raw_data))
  else
...
let sell ~buy_c ~buy_price _ctx _day_k week_k _month_k :
    (Data_cursor.t * float) option Log_warning.LogAndWarnWriter.m =
  let _ = buy_price in
