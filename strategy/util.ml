...
(* 状态A: dif, dea, macd > 0 且 dif > macd, dea > macd *)
...
(* 两种情况: 1. macd 从绿柱过渡到状态A的红柱 2. macd 红柱从左到右竹简变矮后,紧接着变高的第一根红柱 *)
let just_enter_status_A (c : Data_cursor.t) : bool =
  if not (is_status_A c) then
    false
  else
    let current = Data_cursor.current c in
    ...
    | [ l1; l2 ] ->
      if l2.macd < 0. then
        true
      else if l2.macd < current.macd && l1.macd > l2.macd then
        true
      else
        false
    | _ -> false
...
(* return (high_point_c, next_c) next_c: 计算下一个high_point的开始点(跳过macd<0的点) *)
let k_high_point (c : Data_cursor.t) n :
    (Data_cursor.t * Data_cursor.t option) option =
  let rec aux (high_c : Data_cursor.t) continue_low_macd_num (c : Data_cursor.t)
      =
    let current = Data_cursor.current c in
    let high_c_data = Data_cursor.current high_c in
    let high_c' =
      if
        L1.Loader.Type.high current.raw_data
        > L1.Loader.Type.high high_c_data.raw_data
      then
        c
      else
        high_c
    ...
    let continue_low_macd_num' =
      if current.macd < 0. then
        continue_low_macd_num + 1
      else
        0
    in
    if continue_low_macd_num' > n then
      Some (high_c', c)
    else
...
(* 日k高点: 从 c 点开始, 在出现连续 >5 天 macd<0 的情况下, 在这之间的最高点

   return (high_point_c, next_c) next_c: 计算下一个high_point的开始点(跳过macd<0的点) *)
let day_k_high_point c = k_high_point c 5

(* 周k高点: 从 c 开始, 连续 >1周 macd < 0 情况下, 在这之间的最高点
  ...
  | h :: t ->
    let _, r =
      List.fold t
        ...
        ~f:(fun (last_high, r) e ->
          if
            L1.Loader.Type.high (Data_cursor.current e).raw_data
            > L1.Loader.Type.high (Data_cursor.current last_high).raw_data
          then
            (e, e :: r)
          else
            (last_high, r))
  ...
  Data_cursor.fold start ~end' ~init:None ~f:(fun r e ->
      ...
      | Some low ->
        if
          ...
          < L1.Loader.Type.low (Data_cursor.current low).raw_data
        then
          Some e
        else
          r)
...
let%test_module _ =
  ( module struct
    ...
    let month_k = Option.value_exn (L1.Deriving.Unify.unify_month datal)

    let week_k = Option.value_exn (L1.Deriving.Unify.unify_week datal)

    let day_k = Option.value_exn (L1.Deriving.Unify.unify_day datal)
      ...
      let hd =
        List.hd_exn
          (Data_cursor.fold c ~init:[] ~f:(fun r e ->
               if just_enter_status_A e then
                 e :: r
               else
                 r))
      in
      let hd' = (Data_cursor.current hd).date in
      Date.of_string "2019-04-01" = hd'

    let%test "test-day_k_high_point" =
      let c = Data_cursor.create_exn day_k in
      let c' =
        Option.value_exn (Data_cursor.goto_date c (Date.of_string "2019-04-01"))
      in
      let day_k_high, _ = Option.value_exn (day_k_high_point c') in
      Date.of_string "2019-04-24" = (Data_cursor.current day_k_high).date

    let%test "test-week_k_high_point_list" =
      let c = Data_cursor.create_exn week_k in
      let start =
        Option.value_exn (Data_cursor.goto_date c (Date.of_string "2017-01-07"))
      in
      ...
      = List.to_string
          ~f:(fun a -> (Data_cursor.current a).date |> Date.to_string)
          high_list
    ...
    let%test "test-ascending_week_k_high_point_list" =
      ...
      = List.to_string
          ~f:(fun a ->
            ...
            ^ ":"
            ^ string_of_float
                (L1.Loader.Type.high (Data_cursor.current a).raw_data))
          high_list
    ...
    let%test "test-week_k_low_point_list" =
      ...
      = List.to_string
          ~f:(fun a ->
            ...
            ^ ":"
            ^ string_of_float
                (L1.Loader.Type.low (Data_cursor.current a).raw_data))
          low_list
  end )
