...
let status_combine_1 c =
  ...
  let is_down_head =
    List.fold_until fst4 ~init:99999.
      ~f:(fun r e ->
        if L1.Loader.Type.close e.raw_data < r then
          Continue_or_stop.Continue (L1.Loader.Type.close e.raw_data)
        else
...
let%test_module _ =
  ( module struct
    let a1 : L1.Deriving.Type.Derived_data.t =
