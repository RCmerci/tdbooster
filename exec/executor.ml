...
type executorResult =
  | NoAction of
      { name : string
      ; next : Data_cursor.t option
      ; (* next date exec should start *)
...
let to_output name (rl : executorResult list) : Output.t =
  let translist, attentionlist =
    List.map rl ~f:(fun r ->
        ...
        | Transaction { buy; sell; logs; warnings; _ } ->
          ( Some
              ( { i =
                    { date = Data_cursor.datestring (fst buy)
                    ; price = Float.round_decimal ~decimal_digits:2 (snd buy)
                    ; info = []
                    }
                ; o =
                    Option.map sell ~f:(fun sell ->
                        ( { date = Data_cursor.datestring (fst sell)
                          ; price =
                              Float.round_decimal ~decimal_digits:2 (snd sell)
                          ; info = []
                          }
                          : Output.transinfo ))
                ; logs = Strategy.Log_warning.Log.to_string_list logs
                ; warnings =
                    Strategy.Log_warning.Warning.to_string_list warnings
                }
                : Output.transaction )
          , None )
  ...
  [ { name
    ; trans = List.filter_opt translist
    ; attentions = List.filter_opt attentionlist
    }
  ]
...
module Make (St : Strategy.Type.Strategy) = struct
  type t =
    { month_k : Data_cursor.t
    ; week_k : Data_cursor.t
    ; day_k : Data_cursor.t
    }

  let create raw_data =
    L1.Deriving.Unify.unify_month raw_data >>= fun month_k_data ->
    L1.Deriving.Unify.unify_week raw_data >>= fun week_k_data ->
    L1.Deriving.Unify.unify_day raw_data >>= fun day_k_data ->
  ...
  let exec name t =
    ...
    let rec aux ?(logslist = []) ?(warningslist = []) tp f t ctx uplevel_ctx =
      let logs, warnings, v =
        Strategy.Log_warning.LogAndWarnWriter.eval (f t ctx uplevel_ctx)
      in
      match v with
      | Strategy.Type.Buy t' ->
        `Buy
      ...
      | Strategy.Type.Buy_skip_to (ctx', t') -> (
        ...
        | `Month, _ ->
          `Quit
            ...
            , Strategy.Log_warning.Warning.concat (warnings :: warningslist) )
        | _, Some t''' ->
          aux tp f t''' ctx' uplevel_ctx ~logslist:(logs :: logslist)
            ~warningslist:(warnings :: warningslist)
        | _, None ->
          `NotFinished
            ( None
            , Strategy.Log_warning.Log.concat (logs :: logslist)
    ...
    | `NotFinished (_, logs, warnings) ->
      (* month_buy's `NotFinished is nothing *)
      ...
      if n = 0 then
        ...
        Option.value_map
          ...
          ~f:(fun start_week_c ->
            ...
            | `Buy
                ((result_week_c, _, week_to_day_ctx), week_logs, week_warnings)
              ->
              Option.value_map
                ~default:
                  (NoAction
                     { next = Some result_week_c
                     ; logs = Log.concat [ week_logs; month_logs ]
                     ; warnings =
                         Warning.concat [ week_warnings; month_warnings ]
                     ; name
                     })
                (Data_cursor.goto_date t.day_k (Data_cursor.date result_week_c))
                ~f:(fun start_day_c ->
                  ...
                  | `Quit (quit_c, day_logs, day_warnings) ->
                    NoAction
                      ...
                      ; warnings =
                          Warning.concat
                            [ day_warnings; week_warnings; month_warnings ]
                      ; name
                      }
                  ...
                  | `Buy
                      ( (buy_c, buy_price', day_to_sell_ctx)
                      , day_logs
                      , day_warnings ) ->
                    let buy_price = Option.value_exn buy_price' in
                    let logs', warnings', sell =
                      Strategy.Log_warning.LogAndWarnWriter.eval
                    ...
                    Transaction
                      { name
                      ; logs =
                          Log.concat [ logs'; day_logs; week_logs; month_logs ]
                      ; warnings =
                          Warning.concat
                            [ warnings'
                            ; day_warnings
  ...
  let rec exec_aux name t r nextf =
    ...
    | NoAction v -> (
      ...
      | Some next_c -> (
        match
          nextf t (Data_cursor.date next_c) >>| fun t' ->
          exec_aux name t' r nextf
        with
        | None -> r
        | Some v -> v ) )
    | Transaction buysell when Option.is_none buysell.sell -> result :: r
    | Transaction buysell -> (
      ...
      | None -> result :: r )
    | Attention _ -> result :: r

  let exec_sequence name t =
    let goto_next_c t next_c =
      let d = next_c in
      let d' =
        if Data_cursor.date t.day_k >= d then
          Date.add_days (Data_cursor.date t.day_k) 1
        else
          d
      in
      Data_cursor.goto_date t.month_k d' ~hint:`Month >>= fun month_k ->
      Data_cursor.goto_date t.week_k d' ~hint:`Week >>= fun week_k ->
      Data_cursor.goto_date t.day_k d' >>= fun day_k ->
      Some { month_k; week_k; day_k }
    in
