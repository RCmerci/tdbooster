...
(defun tdbooster--render-industry-trend (json subtable-start subtable-length)
  (let* ((data (alist-get 'industry_trend json))
	 ...
	 (datalist (seq-mapn (lambda (e1 e2 e3 e4 e5 e6 e7 e8 e9 e10 e11)
			       (let* ((date1 (seq-elt e1 0))
				      ...
				      (data8 (seq-elt e8 1))
				      (data9 (seq-elt e9 1))
				      (data10 (seq-elt e10 1))
				      (data11 (seq-elt e11 1)))
				 (when (not (and (equal date1 date2)
						 ...
						 (equal date1 date9)
						 (equal date1 date10)
					 	 (equal date1 date11))) (error "bad data"))
				 `(,date1 ,(format "\\cellcolor{gray!%.0f}%.0f" data1 data1)
					  ...
					  ,(format "\\cellcolor{gray!%.0f}%.0f" data11 data11))))
			     ...
			     (seq-subseq (seq-elt (seq-elt data 10) 1) subtable-start subtable-length)))
